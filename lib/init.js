    ...
    _.each(options.providers || {}, function (provider, providerName) {
      ...
      passport.use(new Strategy(provider.config,
        function (req, accessToken, refreshToken, profile, done) {
          ...
          query.$or.push(queryId);

          if (profile.emails) {
            var queryEmail = {'local.emails.0.value': profile.emails[0].value};
            profile.emails[0].verified = true;
            query.$or.push(queryEmail);
          }
          ...
          $user.fetch(function (err) {
            ...
            model.fetch($public, $private, function (err) {
              ...
              if (profile.emails) $private.setNull('local.emails.0', profile.emails[0]);
